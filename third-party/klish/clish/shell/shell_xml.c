 ...
 * but it's super painful otherwise with all these node processing callbacks :(
 */
static const char *g_install_dir = NULL;

static int process_node(clish_shell_t *shell, clish_xmlnode_t *node,
	void *parent);
static int del_cmds_node(clish_shell_t *shell, clish_xmlnode_t *node,
	void *parent);

/* ------------------------------------------------------ */
static int del_process_children(clish_shell_t *shell,
	clish_xmlnode_t *element, void *parent)
{
	...
	while ((node = clish_xmlnode_next_child(element, node)) != NULL) {
		...
		res |= del_cmds_node(shell, node, parent);
	}
...
/* ------------------------------------------------------ */
static int del_process_view(clish_shell_t *shell, clish_xmlnode_t *element,
	void *parent)
{
	clish_view_t *view;
	int res = 0;
	...
	view = clish_shell_find_view(shell, name);

	if (view) {
		res = del_process_children(shell, element, view);
	}
error:
...
/* ------------------------------------------------------ */
static int delete_command(clish_shell_t *shell, clish_xmlnode_t *element,
	void *parent)
{
	...
	fprintf(stderr, "BFShell: Command %s will be unavailable due to missing components.\n", name);
	clish_view_delete_command(v, name);

	res = del_process_children(shell, element, cmd);
error:
	clish_xml_release(name);
...
/*
 * ------------------------------------------------------
 * This function reads an element from the XML stream and
 * deletes all commands under it
 * ------------------------------------------------------
 */
static int del_cmds_node(clish_shell_t *shell, clish_xmlnode_t *node,
	void *parent)
...
#ifdef DEBUG
					fprintf(stderr, "DEL NODE:");
					clish_xmlnode_print(node, stderr);
					fprintf(stderr, "\n");
#endif
					delete_command(shell, node, parent);
				} else if (0 == strcmp(name, "VIEW")) {
					del_process_view(shell, node, parent);
				} else {
					del_process_children(shell, node, parent);
				}
			}
			break;
		}
...
/*-------------------------------------------------------- */
static int valid_p4_check(const char *fname, char **p4_names)
{
	if (strcmp(fname, "switchapi.xml") == 0) { /* hack to deal with switchapi xml */
		fname = "pd_switchapi~.xml";
	}

    if ((strcmp(fname, "bf_switchapi.xml")) == 0) {
        fname = "pd_bf_switchapi~.xml";
    }
	size_t lenFname = strlen(fname);
	if (lenFname < 4) return 1;
	if (strncmp(fname, "pd_", 3) != 0) return 1;

	int cur_p4 = 0;
	char *fname_cpy = bf_sys_strdup(fname);
	*strchr(fname_cpy, '.') = '\0';
	const char *p4_fname = fname_cpy + 3;
	while (p4_names[cur_p4]) {
		if (strcmp(p4_fname, p4_names[cur_p4]) == 0) {
			bf_sys_free(fname_cpy);
			return 1;
		}
		cur_p4++;
	}
...
/*-------------------------------------------------------- */
int clish_shell_load_scheme(clish_shell_t *this, const char *install_dir_path, char **p4_names, const char *xslt_path)
{
	char abs_path[1024] = {0};
	const char *path = NULL;
	if (install_dir_path) {
		snprintf(abs_path, 1023, "%sshare/cli/xml", install_dir_path);
		path = abs_path;
	}
	g_install_dir = install_dir_path;
	char *buffer;
	char *dirname;
	char *saveptr = NULL;
	int res = -1;
	...
	for (dirname = strtok_r(buffer, ";", &saveptr);
		...
		for (entry = readdir(dir); entry; entry = readdir(dir)) {
			...
			/* Filter invalid p4 */
			if (valid_p4_check(entry->d_name, p4_names) == 0) {
				continue;
			}
	...
	void *parent)
{
	 ...
	 */
	do {
		char *new = (char*)lub_realloc(content, content_len);
		if (!new) {
			if (content)
				lub_free(content);
			return -1;
	...
	void *parent)
{
	...
	if (file && *file) {
		char abs_path[1024] = {0};
		snprintf(abs_path, 1023, "%s%s", g_install_dir, file);
		clish_plugin__set_file(plugin, abs_path);
	}
	...
	res = clish_plugin_load(plugin, (void *)shell);
	if (res) {
		clish_plugin_free(plugin, (void*)shell);
		goto error;
	}
