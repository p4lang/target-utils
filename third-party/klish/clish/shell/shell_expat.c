...
static clish_xmlnode_t *clish_expat_add_attr(clish_xmlnode_t *first, const char *n, const char *v)
{
	...
	node = lub_malloc(sizeof(clish_xmlnode_t));
	if (!node)
		return first;

	node->name = lub_strdup(n);
	node->content = lub_strdup(v);
	node->children = NULL;
					      ...
					      const char **attr)
{
	...
	if (!node)
		return NULL;
	node->name = n ? lub_strdup(n) : NULL;
	node->content = v ? lub_strdup(v) : NULL;
	node->children = NULL;
	node->attributes = NULL;
	node->next = NULL;
	node->parent = parent;
...
clish_xmldoc_t *clish_xmldoc_read(const char *filename)
{
	...
	doc = lub_malloc(sizeof(clish_xmldoc_t));
	if (!doc)
		return NULL;
	memset(doc, 0, sizeof(clish_xmldoc_t));
	doc->filename = lub_strdup(filename);
	mhs.malloc_fcn = lub_malloc;
	mhs.realloc_fcn = lub_realloc;
	mhs.free_fcn = lub_free;
	...
	if (fd < 0)
		goto error_open;
	fstat(fd, &sb);
	buffer = lub_malloc(sb.st_size+1);
