...
clish_shell_t *clish_shell_new(
	...
	const char *install_dir)
{
	clish_shell_t *this = lub_malloc(sizeof(clish_shell_t));

	if (this)
		clish_shell_init(this, istream, ostream, stop_on_error, install_dir);
...
/*--------------------------------------------------------- */
const char *clish_shell__get_install_dir(clish_shell_t * this)
{
