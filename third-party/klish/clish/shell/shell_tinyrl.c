...
static void print_help(tinyrl_t *this) {
...
static bool_t clish_shell_tinyrl_key_space(tinyrl_t *this, int key)
{
	...
	if (result)
		...
		tinyrl_crlf(this);
		tinyrl_printf(this, "There is no valid command with that prefix.");
		tinyrl_crlf(this);
		tinyrl_reset_line_state(this);
	}
	/* keep compiler happy */
...
static bool_t clish_shell_tinyrl_key_enter(tinyrl_t *this, int key)
{
	...
	if (!cmd) {
		...
		default:
			...
			tinyrl_crlf(this);
			tinyrl_printf(this, "Invalid/incomplete command.");
			tinyrl_crlf(this);
			tinyrl_reset_line_state(this);
			break;
		}
	...
	if (errmsg) {
		if (tinyrl__get_isatty(this) || !shell->current_file) {
			tinyrl_printf(this, "Syntax error: %s\n", errmsg);
			tinyrl_reset_line_state(this);
		} else {
			char *fname = "stdin";
			FILE *ostream = tinyrl__get_ostream(this);
			if (shell->current_file->fname)
				fname = shell->current_file->fname;
			fprintf(ostream, "Syntax error on line %s:%u \"%s\": "
