...
clish_plugin_t *clish_plugin_new(const char *name)
{
	...
	this->fini = NULL;
	/* Flags */
	this->rtld_global = BOOL_TRUE; /* The dlopen() not use RTLD_LOCAL by default */
...
static int clish_plugin_load_shared(clish_plugin_t *this)
{
	...
	if (this->file) {
		...
		lub_string_cat(&file, "bfshell_plugin_");
  ...
  if (strcmp(this->name, "clish") == 0) {
    this->init = CLISH_PLUGIN_INIT_FNAME(clish);
  } else if (strcmp(this->name, "pipemgr") == 0) {
    this->init = CLISH_PLUGIN_INIT_FNAME(pipemgr);
  /*
  } else if (strcmp(this->name, "debug") == 0) {
    this->init = CLISH_PLUGIN_INIT_FNAME(debug);
  } else if (strcmp(this->name, "bf_rt") == 0) {
    this->init = CLISH_PLUGIN_INIT_FNAME(bf_rt);
  */
  } else if (strcmp(this->name, "bf_switchapi") == 0) {
    this->init = CLISH_PLUGIN_INIT_FNAME(bf_switchapi);
  }
...
int clish_plugin_load(clish_plugin_t *this, void *userdata)
{
	...
	if (!this->builtin_flag) {
		if (this->init) {
			// We've already loaded this plugin
			return 0;
		}
