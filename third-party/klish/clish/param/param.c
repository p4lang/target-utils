...
{
	clish_param_t *this = lub_malloc(sizeof(clish_param_t));
...
{
	clish_paramv_t *this = lub_malloc(sizeof(clish_paramv_t));
...
void clish_paramv_insert(clish_paramv_t * this, clish_param_t * param)
{
	...
	tmp = lub_realloc(this->paramv, new_size);
	if (tmp) {
		this->paramv = tmp;
