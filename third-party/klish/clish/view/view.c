...
{
	clish_view_t *this = lub_malloc(sizeof(clish_view_t));
...
/*--------------------------------------------------------- */
void clish_view_delete_command(clish_view_t * this,
	const char *name)
{
	clish_command_t *cmd = clish_view_find_command(this, name, BOOL_FALSE);
	if (!cmd) {
		return;
	}
	/* remove the command from the tree */
