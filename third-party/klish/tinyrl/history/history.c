...
{
	tinyrl_history_t *this = lub_malloc(sizeof(tinyrl_history_t));
	if (NULL != this) {
...
static void add_n_grow(tinyrl_history_t * this, const char *line)
{
	if (this->size == this->length) {
		...
		new_entries = lub_realloc(this->entries, nbytes);
		if (NULL != new_entries) {
			this->size = new_size;
