...
static void lub_argv_init(lub_argv_t * this, const char *line, size_t offset)
{
	...
	arg = this->argv = lub_malloc(sizeof(lub_arg_t) * this->argc);
	assert(arg);
...
lub_argv_t *lub_argv_new(const char *line, size_t offset)
{
	...
	this = lub_malloc(sizeof(lub_argv_t));
	if (this)
...
void lub_argv_add(lub_argv_t * this, const char *text)
{
	...
	/* allocate space to hold the vector */
	arg = lub_realloc(this->argv, sizeof(lub_arg_t) * (this->argc + 1));
	assert(arg);
	this->argv = arg;
	(this->argv[this->argc++]).arg = lub_strdup(text);
}
...
char **lub_argv__get_argv(const lub_argv_t * this, const char *argv0)
{
	...
	result = lub_malloc(sizeof(char *) * (this->argc + 1 + a));

	if (argv0)
		result[0] = lub_strdup(argv0);
	for (i = 0; i < this->argc; i++)
		result[i + a] = lub_strdup(this->argv[i].arg);
	result[i + a] = NULL;
