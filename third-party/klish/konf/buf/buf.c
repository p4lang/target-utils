...
{
	this->fd = fd;
	this->buf = lub_malloc(KONF_BUF_CHUNK);
...
{
	konf_buf_t *this = lub_malloc(sizeof(konf_buf_t));
...
static int konf_buf_realloc(konf_buf_t *this, int addsize)
{
	...
	if ((this->size - this->pos) < chunk) {
		tmpbuf = lub_realloc(this->buf, this->size + chunk);
		this->buf = tmpbuf;
		this->size += chunk;
	}
...
char * konf_buf_parse(konf_buf_t *this)
{
	...
	if ((this->size - this->pos) > (2 * KONF_BUF_CHUNK)) {
		char *tmpbuf;
		tmpbuf = lub_realloc(this->buf, this->size - KONF_BUF_CHUNK);
		this->buf = tmpbuf;
		this->size -= KONF_BUF_CHUNK;
	}
