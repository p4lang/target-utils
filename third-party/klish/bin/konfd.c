...
int main(int argc, char **argv)
{
    ...
    */
	struct sigection sigpipe_act = {0};
...
struct options *opts_init(void)
{
	...
	opts = lub_malloc(sizeof(*opts));
	assert(opts);
	opts->debug = 0; /* daemonize by default */
	opts->socket_path = lub_strdup(KONFD_SOCKET_PATH);
	opts->ro_path = NULL;
	opts->pidfile = lub_strdup(KONFD_PIDFILE);
	opts->chroot = NULL;
	opts->uid = getuid();
	opts->gid = getgid();
...
{
	if (opts->socket_path)
		lub_free(opts->socket_path);
	if (opts->ro_path)
		lub_free(opts->ro_path);
	if (opts->pidfile)
		lub_free(opts->pidfile);
	if (opts->chroot)
		lub_free(opts->chroot);
	lub_free(opts);
}
...
static int opts_parse(int argc, char *argv[], struct options *opts)
{
	...
	while(1) {
		...
		case 's':
			if (opts->socket_path)
				lub_free(opts->socket_path);
			opts->socket_path = lub_strdup(optarg);
			break;
		case 'S':
			if (opts->ro_path)
				lub_free(opts->ro_path);
			opts->ro_path = lub_strdup(optarg);
			break;
		case 'p':
			if (opts->pidfile)
				lub_free(opts->pidfile);
			opts->pidfile = lub_strdup(optarg);
			break;
		case 'r':
#ifdef HAVE_CHROOT
			if (opts->chroot)
				lub_free(opts->chroot);
			opts->chroot = lub_strdup(optarg);
#else
