...
int
ucli_node_remove(ucli_t* ucli, ucli_node_t* node)
{
    if(ucli == NULL || node == NULL) {
        return -1;
    }
    return ucli_node_subnode_remove(ucli->root, node);
}
...
char*
ucli_complete_hook(ucli_t* ucli, const char* line, aim_pvs_t *pvs)
{
    ucli_completion_t* c = ucli_complete_string(ucli, line);
    char* s;
    aim_pvs_t *term_pvs = pvs;
        ...
        if(c->append_string) {
            ...
            if(c->possibilities.list) {
                ...
                BIGLIST_FOREACH_DATA(ble, c->possibilities.list, char*, s) {
                    aim_printf(term_pvs, "    %s\n", s);
                }
            }

            if(c->help_node) {
                aim_printf(term_pvs, "\n");
                ucli_node_help(c->help_node, term_pvs);
            }
            ucli_completion_free(c);
            return NULL;
        }
...
int
ucli_run(ucli_t* ucli, const char* prompt, FILE *fin, FILE *fout, FILE *ferr)
{
    aim_pvs_t *pvs = NULL;
    if (fout == stdout) {
        pvs = &aim_pvs_stdout;
    ...
    pvs->object.cookie = fout;
    els_t* els = els_create("ucli", ucli_els_prompt__, ucli, fin, fout, ferr);
    els_complete_set(els, (els_complete_f)ucli_complete_hook, ucli, pvs);
    els_start(els);
    ...
    for(;;) {
        ...
        if(line) {
            if(line[0] == '^' && line[1] == 'D') {
                ...
                if (ucli_dispatch_string(ucli, pvs, line) == UCLI_STATUS_E_EXIT) {
                    break;
                }
            }
        }
        if ((!line) && (!els->std_streams)) {
            printf("bf-shell ucli exiting as EOF was received \n");
            break;
        }
    }
    /* Reset the stream output */
    pvs->object.cookie = NULL;
    els_destroy(els);
    return 0;
}
